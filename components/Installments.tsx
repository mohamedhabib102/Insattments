...
export default function Installments({ refreshTrigger = 0 }: InstallmentsProps) {
    ...
    const [originalInstallments, setOriginalInstallments] = useState<InstallmentRequest[]>([]);
    const [filteredInstallments, setFilteredInstallments] = useState<InstallmentRequest[]>([]);
    ...
    const fetchInstallments = async () => {
        ...
        try {
            ...
            console.error("Error fetching originalInstallments:", error);
    ...
    const handelSearch = (query: string) => {
        ...
        if (!query.trim()) {
            setFilteredInstallments(originalInstallments);
            return;
        }

        const filtered = originalInstallments.filter(item =>
    ...
    const GetInstallmentByFilter = async (value: string) => {
        const id = Number(userData?.userId);
        setLoading(true);
        try {
            const res = await api.get(`/api/Evaluation App/GetInstallmentsByFilter?personId=${id}&filterType=${value}`);
    ...
    return (
        <>
            <div className="mt-8 space-y-8">
                ...
                w-full p-4 text-lg border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`
                        }
                        value={searchQuery}
                                        ...
                                        <div className="flex justify-between items-start mb-6">
                                            <div>
                                                ...
                                                <p className="font-bold text-gray-900">
                                                    {`${new Date(item.dueDate).getDate()}/${new Date(item.dueDate).getMonth() + 1}/${new Date(item.dueDate).getFullYear()}`}
                                                </p>
                                            </div>
                                            <div>
